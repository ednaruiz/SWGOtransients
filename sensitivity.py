...
class HAWCSensi:
    ...
    '''
    energy = np.array([0.17782794,0.56234133,1.77827941,5.62341325,17.7827941,56.23413252])*1.6 #in TeV
    ...
    def read_data(self,file):
        ...
        df["Emin"] = np.array([0.1,0.31622777,1.,3.16227766,10.,31.6227766])*1.6 #units of TeV
        df["Emax"] = np.array([0.31622777,1.,3.16227766,10.,31.6227766,100.])*1.6 #units of TeV
        ...
        for icol in ["50deg","40deg","30deg","20deg","10deg","0deg"]:
            ...
            df[icol] = df[icol]/ (df["Energy"]*df["Energy"]) #now we will have units of erg-1 cm-2 s-1
    ...
    def interpolate(self, z, e, t):
        ...
        #convert the input energy to ergs since all the interpolation functions use ergs
    ...
    def sensitivity_vs_time(self,zenith, emin, emax, times ):
        ...
        nbins = 30
        e_bins = np.logspace(np.log10(emin),np.log10(emax),nbins) #in ergs
        ...
        for it in times:
            ...
            for ie in e_bins:
                #intperolate gives dnde, multiply by E to have Ednde
                per_time.append((self.interpolate(zenith,ie/1.6,it))*ie)
            #we do the sum of the numerical integration here
            s_vs_time.append(np.sum( np.array(per_time)* (emax-emin)/float(nbins))    ) #this integration will give an additional energy unit to have finaly E2dnde
